...
// => Properties 파일에서는 영문자,숫자,일부 특수문자를 제외한 나머지 문자는 유니코드로 변한되어 처리
...
public class PropertiesApp {
	public static void main(String[] args) throws Exception {
		...
		//java.utill 패키지의 Properties 클래스로 객체 생성
		// => Properties 객체 : Properties 파일의 내용을 전달받아 저장하기 위한 객체
		// => Map 인터페이스를 상속받은 자료구조 클래스 - 다수의 엔트리(Entry - Key와 Value) 저장 
		Properties properties=new Properties();
		
		//Properties.load(InputStream in) : 입력스트림을 사용하여 Properties 파일의 내용(이름과 값)을 
		//제공받아 Properties 객체의 엔트리로 저장하는 메소드
		properties.load(in);
		
		//Properies.get(String key) : Properties 객체에 저장된 엔트리에서 맵키(MapKey)를 전달받아
