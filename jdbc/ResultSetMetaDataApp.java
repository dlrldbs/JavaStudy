...
//STUDENT 테이블에 저장된 모든 학생정보를 검색하여 출력하는 JDBC
public class ResultSetMetaDataApp {
	public static void main(String[] args) throws SQLException {
		...
		System.out.println("=============================================================");
		//ResultSet.getMetaData() : 검색결과(ResultSet 객체)에 대한 부가적인 정보를 저장하는
		...
		for(int i=1;i<=columnCount;i++) {
			//ResultSetMetaData.getColumnLabel(int columnIndex) : columnIndex 위치의 칼럼명을 반환
			...
			//ResultSetMetaData.columnNoNulls : NULL을 허용하지 않는 값을 표현하는 상수(Constant) - 정수값 : 0
			if(isNull==ResultSetMetaData.columnNoNulls) {
				nullResult="NOT NULL";
			}
			
			//ResultSetMetaData.getColumnTypeName(int columnIndex) : columnIndex 위치의 컬럼에 대한
			//오라클 자료형을 반환하는 메소드
			...
			System.out.println("컬럼 자료형 = "+columTypeName);
