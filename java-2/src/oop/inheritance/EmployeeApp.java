...
public class EmployeeApp {
	public static void main(String[] args) {
		...
		//추상클래스(부모클래스)로 참조변수를 생성하여 자식클래스의 객체 저장 가능
		...
		for(Employee employee:empArray) {
			...
			/*
			//사원의 급여를 반환받아 출력
			// => 참조변수로 부모클래스의 객체만 참조 가능하므로 자식클래스의 메소드 호출 불가능
			// => 명시적 객체 형변환 사용하여 참조변수로 자식클래스의 객체를 참조하여 메소드 호출
			// => instanceof 연산자를 사용하여 자식클래스를 구분하여 객체 형변환 - ClassCastException
