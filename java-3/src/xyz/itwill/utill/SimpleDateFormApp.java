...
//java.text 패키지 : 값을 변환하는 기능 제공하는 클래스가 선언된 패키지
...
public class SimpleDateFormApp {
	public static void main(String[] args) {
		...
		//SimpleDateFormat 객체에 저장된 패턴정보를 이용하여 문자열로 변화하여 반환하는 메소드
		String printDate=dateFormat.format(now);
		
		System.out.println("현재 = "+printDate);
		
		//SimpleDateFormat.applyPattern(String pattern) : SimpleDateFormat 객체에 저장된 패턴값
		dateFormat.applyPattern("yyyy-MM-dd HH:mm:ss");
		System.out.println("현재 = "+dateFormat.format(now));
		
		String want="2000-01-01 00:00:00.00";
		try {
			//SimpleDateFormat.parse(String str) : SimpleDateFormat 객체에 저장된 패턴정보
			//형식의 문자열을 매개변수로 전달받아 Date 객체로 변환하여 반환하는 메소드
			// => SimpleDateFormat 객체에 저장된 패턴정보와 일치하지 않는 문자열을 매개변수에 
			//전달한 경우 ParseException 발생 
