...
// => 난수와 입력값을 서로 비교하여 동일한 정수값이 존재할 경우 위치가 같으면 스트라이크 처리하고 
// 다르면 볼로 처리하여 계산
...
public class BaseBallApp {
	public static void main(String[] args) {
		...
		while(true) {
			...
			//규칙에 맞는 입력값이 경우에만 반복문 종료
			if(dap[0]!=0 && dap[0]!=dap[1] && dap[1]!=dap[2] && dap[2]!=dap[0]) break;	
		}
		
		Scanner scanner=new Scanner(System.in);
		
		//0~9 범위의 정수값 3개를 저장하기 위한 배열 선언 - 키코브 입력값을 저장하기 위한 배열
		int[] num=new int[3];
		...
		for(int cnt=1;cnt<=15;cnt++) {
			...
			if(strike==3) {
				System.out.println("[메세지]축하합니다"+cnt+"번째 만에 맞췄습니다.");
				break;
			}
