...
//세션 트렉킹(Session Tracking) : [JSESSIONID] 이름의 쿠키값을 서버에 저장된 세션의 아이디와 
//비교하여 검색하는 작업
...
public class SessionServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//HttpSession 객체 : 세션 정보를 저장한 객체 
		// => 연결 지속성을 제공하기 위한 값을 Map 형식(이름과 값을 하나의 요소로 표현)으로 저장
		...
		out.println("<hr>");
		//HttpSession.isNew() : 세션을 트렉킹하여 바인딩된 경우 [false]를 반환하고 세션을
		//생성하여 바인딩 경우 [true]를 반환하는 메소드
		if(session.isNew()) {
		...
		// => 바인딩 세션에 세션 속성명(문자열)과 세션 속성값(객체)을 저장(변경)하는 메소드
		// => 기존 세션에 저장된 값과 같은 이름의 속성명을 사용하는 경우 속성값 변경
		...
		session.setAttribute("now", new Date()); //다른 웹프로그램에서 사용할 수 있도록 객체 공유
		
		//HttpSession.getAttribute(String attributeName) : 바인딩된 세션에 저장된 속성값을 
		//속성명을 전달받아 반환하는 메소드
		...
		//HttpSession.removeAttribute(String attributeName) : 바인딩된 세션에 저장된 속성값을
		//속성명을 전달받아 삭제하는 메소드
