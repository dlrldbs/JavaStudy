...
public class DateApp {
	...
	public static void main(String[] args) {
		//Date 클래스의 기본 생성자로 객체를 생성하면 시스템의 현재 날짜와 시간이 저장된 Date 
		Date now=new Date();
		...
		// => 1900년도부터 1씩 증가된 정수값 반환
		//Date.getMonth() : Date 객체에 저장된 날짜와 시간에서 월을 반환하는 메소드
		// => 0(1월)~11(12월) 범위의 정수값 반환
		//Date.getDate() : Date 객체에 저장된 날짜와 시간에서 일을 반환하는 메소드
		//Date.getDay() : Date 객체에 저장된 날짜와 시간에서 요일을 반환하는 메소드
		// => 0(일)~6(토) 범위의 정수값 반환
		String printDate=(now.getYear()+1900)+"년 "+(now.getMonth()+1)+"월 "+now.getDate()+"일 "+day[now.getDay()]+"요일";
		System.out.println("현재 = "+printDate);
		
		//Date.getTime() : Date 객체에 저장된 날짜와 시간을 long 자료형의 정수값(타임스탬프 - TimeStamp)으로 반환하는 메소드
		//타임스탬프(TimeStamp) : 1970년 1월 1일을 기준으로 1ms마다 1씩 증가된 정수값
		...
		Date wantDate=new Date(100,0,1);//[2000년 1월 1일 0시 0분 0초]가 저장된 Date 객체 생성
		System.out.println("wantDate = "+wantDate);
		
		long wantTime=wantDate.getTime();
		
		System.out.println("연산결과(일"
				+ ") = "+((nowTime-wantTime)/1000)/86400);
	}
}
