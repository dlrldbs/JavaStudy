...
// => 블럭 안의 명령을 1번 이상 반복 실행 - 조건식의 결과가 처음부터 거짓인 경우에도 명령 실행
...
public class WhileApp {
	public static void main(String[] args) {
		...
		while(gae<500) {//A4용지를 반으로 접는 행위를 반복 처리
			cnt++;//접는 횟수 1씩 증가
			gae*=2;//사각형 모양의 갯수는 2배씩 증가
		}
		System.out.println(cnt+"번 접으면 "+gae+"개의 사각형이 만들어집니다.");
		System.out.println("====================================================");
		//1~X 범위의 점수들의 합계가 300 이상이 만들어지려면 X가 얼마인지 계산하여 출력
		int x=1,sum=0; //x : 1씩 증가되는 정수값 저장, sum : 누적결과(합계) 저장
		while(sum<300) {
			x++;
			sum+=x;
		}
