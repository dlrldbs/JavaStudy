...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 입력페이지(join_form.jsp)에서 전달된 회원정보를 반환받아 클라이언트에게 전달하는 JSP 문서 - 처리페이지 --%>
<%
	//비정상적인 요청에 대한 처리 - 클라이언트에게 에러코드 전달하거나 에러페이지(입력페이지)로
	//이동하기 위한 URL 주소 전달
	//request.getMethod() : 클라이언트의 요청방식(GET or POST)을 반환하는 메소드
	if(request.getMethod().equals("GET")){//JSP문서를 [GET]방식으로 요청한 경우 - 비정상적인 요청
		/*
		...
		*/
		...
		// => 클라이언트는 요청 URL 주소를 전달받아 브라우저의 URL 주소를 변경하고 웹프로그램을
		//요청하여 결과를 응답받아 출력 - 리다이렉트 이동
		// => 리다이렉트 이동은 URL 주소는 서버의 최상위 디렉토리를 이용하여 절대경로로 표현 가능
		...
		*/
		...
		out.println("alert('비정상적인 방법으로 요청하였습니다.');");		
		...
		*/
		...
		response.sendRedirect("join_form.jsp?message="+message);//입력페이지 이동
		return;
		*/
		...
		*/
		...
		session.setAttribute("message", "비정상적인 방법으로 요청하였습니다.");
	...
	//대한 문자형태(CharacterSet)를 변경하는 메소드
	...
	//request.getParameterValues(String name) : 같음 이름으로 전달되는 모든 값을 
	//문자열(String 객체) 배열로 반환하는 메소드 
