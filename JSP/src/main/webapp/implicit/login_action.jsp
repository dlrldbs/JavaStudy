<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%-- 로그인 입력페이지(login_form.jsp)에서 전달된 인증정보를 반환받아 인증 처리하고 처리결과에 따라
 다른 다른 요청 URL 주소를 클라이언트에게 전달하는 JSP 문서 --%>
<%-- => 로그인(login) 인증을 통한 권한 획득 과정 --%>
<%-- 인증 실패 : 클라이언트 입력페이지(login_form.jsp)로 이동되도록 URL 주소 전달 --%>
<%-- 인증 성공 : 클라이언트 출력페이지(login_user.jsp)로 이동되도록 URL 주소 전달 --%>
